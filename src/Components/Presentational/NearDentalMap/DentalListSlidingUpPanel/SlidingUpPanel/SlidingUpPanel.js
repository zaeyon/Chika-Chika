...
class SlidingUpPanel extends React.PureComponent {
  static propTypes = {
    ...
    slidingUpPanel: PropTypes.func,
    slidingDownPanel: PropTypes.func,
    listIndex: PropTypes.number,
    panelState: PropTypes.string,
    dragDentalPanel: PropTypes.func,
  }
  ...
  _onMoveShouldSetPanResponder(evt, gestureState) {
    ...
    if (!this.props.allowDragging) {
      if((this.props.listIndex == 0 || this.props.listIndex == -0) && gestureState.dy > 0) {
        return true
      } else {
        return false
      }
  ...
  _onPanResponderMove(evt, gestureState) {
    ...
    this.props.animatedValue.setValue(newValue)
    this.props.dragDentalPanel(gestureState)
  }

  // Trigger when you release your finger
  _onPanResponderRelease(evt, gestureState) {
    console.log("this.props.list", this.props.listIndex);

    if(this.props.listIndex == "0" || this.props.listIndex == "-0") {
      console.log("패널 숨기기")
      this.hide()
      this.props.slidingDownPanel()
    }

    console.log("onPanResponderRelease gestureState", gestureState);
    if(gestureState.dy < 0) {
      this.props.slidingUpPanel()
     const animatedValue = this.props.animatedValue.__getValue()
    ...
    if (this.props.snappingPoints.length > 0) {
      console.log("this.props.snappingPoints");
    ...
    if (Math.abs(gestureState.vy) > this.props.minimumVelocityThreshold) {
      console.log("Math.abs(gestureState.vy) > this.props.minimumVelocityThreshold")
      if(gestureState.dy < 0) {
        this.show()
      }
      this.props.onMomentumDragStart(animatedValue)
