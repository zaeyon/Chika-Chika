...
const NotificationListScreen = ({navigation, route}: Props) => {
  ...
  const [likeNotificationArray, setLikeNotificationArray] = useState<
    Array<any>
  >([]);
  const [eventNotificationArray, setEventNotificationArray] = useState<
    Array<any>
  >([]);
  const [commentNotificationArray, setCommentNotificationArray] = useState<
    Array<any>
  >([]);
  const [changeNotificationArray, setChangeNotificationArray] = useState<
    boolean
  >(false);

  const [likeNotifyIdArray, setLikeNotifyIdArray] = useState<Array<any>>([
    'null',
  ]);
  const [eventNotifyIdArray, setEventNotifyIdArray] = useState<Array<any>>([
    'null',
  ]);
  const [commentNotifyIdArray, setCommentNotifyIdArray] = useState<Array<any>>([
    'null',
  ]);

  const [loadingGetLikeNotify, setLoadingGetLikeNotify] = useState<boolean>(
    true,
  );
  const [loadingGetCommentNotify, setLoadingGetCommentNotify] = useState<
    boolean
  >(true);
  const [loadingGetEventNotify, setLoadingGetEventNotify] = useState<boolean>(
    true,
  );
  ...
  like: [
    {
      ...
      user: {
        profileImageUri:
          'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRoU0TZbbxdkql-91CkCAXg13dAZMVqlaz7CQ&usqp=CAU',
        nickname: '알림테스트',
      },
  ...
  event: [
    {
      ...
      user: {
        profileImageUri:
          'http://hil.or.kr/wp-content/uploads/2018/08/%EB%8C%80%EC%A0%84%EC%84%A0%EC%B9%98%EA%B3%BC%EB%B3%91%EC%9B%9001s.jpg',
        nickname: '광교좋은날치과병원',
      },
    ...
    },
    {
      ...
      description:
        '치과병원은 구강악안면외과, 치과보존과, 치과보철과, 소아치과, 치과교정과, 치주과 6개 영역으로 진료영역을 특화해 최상의 치료를 제공합니다. 각 진료과는 고유의 영역을 전문적·독립적으로 진료하며, 동시에 환자 상태에 따라 관련 과와 협진하여 환자 중심의 맞춤 진료를 진행합니다.',
      data: '2021-01-25',
      id: 7,
    },
    {
  ...
  comment: [
    {
      ...
      user: {
        profileImageUri:
          'https://cdn.ajoumc.or.kr/Upload/MedicalCenter/Doctor/Profile/201812/20181203133458927.jpg',
        nickname: '치과의사',
      },
