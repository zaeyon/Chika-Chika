...
const DetailPriceScreen = ({navigation, route}: Props) => {
  ...
  const [changeDetailPriceList, setChangeDetailPriceList] = useState<boolean>(
    false,
  );
  ...
  const onChangeDetailPriceInput = (text: string, index: number) => {
    if (text.length > 0) {
      ...
      tmpSelectedTreatList[index].displayPrice =
        Number(text).toLocaleString() + '원';
          ...
          <DetailPriceInputContainer
            ...
            <DetailPriceInput
              ...
              onChangeText={(text: String) =>
                onChangeDetailPriceInput(text, index)
              }
            />
