...
const TEST_REVIEW_DATA = [
  {
    ...
    reviewImages: [
      {
        uri:
          'http://cfs9.tistory.com/image/15/tistory/2008/09/26/14/31/48dc73c30853d',
      },
      {
        uri:
          'http://cfs9.tistory.com/image/15/tistory/2008/09/26/14/31/48dc73c30853d',
    ...
    rating: '3.5',
    description:
      '교정치과에서 악궁확장장치를 달고 온 날이예요. 그 전에 공포의 파란 고무링을 어금니 사이마다 좌우 3개씩 총 6개를 2주 정도 끼워서 어금니 사이를 벌려요.',
    view: '300',
    getInfo: '102',
    like: '123',
    comment: '24',
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        <ContentContainer>
          ...
          <SelectInfoTypeContainer>
            ...
            <TouchableWithoutFeedback onPress={() => clickReviewType()}>
              <SelectInfoTypeText style={{marginLeft: 29}}>
                {'리뷰'}
              </SelectInfoTypeText>
            </TouchableWithoutFeedback>
          </SelectInfoTypeContainer>
          {infoType === 'detailInfo' && (
            <DentalDetailInfoContainer>
              <IntroContainer>
                ...
                <InfoValueText>
                  {dentalDetailInfo.detailInfo.introduction}
                </InfoValueText>
              </IntroContainer>
              <EstablishDateContainer>
                <InfoLabelText>{'설립일자'}</InfoLabelText>
                <InfoValueText>
                  {dentalDetailInfo.detailInfo.establishDate}
                </InfoValueText>
              </EstablishDateContainer>
              <TreatTimeContainer>
                <InfoLabelText>{'진료시간'}</InfoLabelText>
                <InfoValueText>
                  {dentalDetailInfo.detailInfo.treatTime}
                </InfoValueText>
                <WeeklyTreatTimeContainer></WeeklyTreatTimeContainer>
              </TreatTimeContainer>
              <LocationInfoContainer>
                <InfoLabelText>{'위치정보'}</InfoLabelText>
                <InfoValueText>
                  {dentalDetailInfo.detailInfo.address}
                </InfoValueText>
                <LocationMapContainer>
                  <LocationMapImage source={{uri: locationMapBase64}} />
                </LocationMapContainer>
              </LocationInfoContainer>
              <TreatUniquenessContainer>
                <InfoLabelText>{'진료 특이사항'}</InfoLabelText>
                <InfoValueText>
                  {dentalDetailInfo.detailInfo.uniqueness}
                </InfoValueText>
              </TreatUniquenessContainer>
              <DentistInfoContainer>
                <InfoLabelText>{'의사정보'}</InfoLabelText>
                <InfoValueText>
                  {dentalDetailInfo.detailInfo.dentist}
                </InfoValueText>
              </DentistInfoContainer>
              <ManageContainer>
                <TouchableWithoutFeedback
                  onPress={() => moveToDentalInfoEdit()}>
                  <EditInfoButton>
                    <ManageText>{'정보수정'}</ManageText>
                  </EditInfoButton>
                </TouchableWithoutFeedback>
                <MakeDentalAccountButton>
                  <ManageText>{'무료병원계정'}</ManageText>
                </MakeDentalAccountButton>
              </ManageContainer>
            </DentalDetailInfoContainer>
          )}
              ...
              <AverageRatingContainer>
                ...
                <RatingItemListContainer style={{marginTop: 8}}>
                  <RatingItemContainer>
                    ...
                    <RatingValueText style={{marginTop: 14}}>
                      {'★4.0 / 5.0'}
                    </RatingValueText>
                  </RatingItemContainer>
                  <RatingItemContainer>
        ...
        <FooterContainer>
          <ReserveByPhoneContainer>
            <TouchableWithoutFeedback
              onPress={() => callPhoneNumber('01093664131')}>
