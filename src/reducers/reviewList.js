const reviewList = (state = {mainReviewList: [], MyReviews: []}, action) => {
  switch (action.type) {
    ...
    case 'TOGGLE_REVIEW_LIKE': {
      ...
      if (targetIndex !== -1) {
        const liked =
          tmpReviewList[targetIndex].viewerLikedReview == 1 ? true : false;
        tmpReviewList[targetIndex].viewerLikedReview = liked ? 0 : 1;
        tmpReviewList[targetIndex].reviewLikeNum = liked
          ? tmpReviewList[targetIndex].reviewLikeNum - 1
          : tmpReviewList[targetIndex].reviewLikeNum + 1;
      }
